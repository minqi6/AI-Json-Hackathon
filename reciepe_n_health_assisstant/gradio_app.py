...
def ingredient_breakdown(food):
    result = subprocess.run(
            ['python', 'reciepe_n_health_assisstant/flow_ingredient_breakdown.py', food],
...
def recipe_recommendation(category, dietary_restrictions):
    result = subprocess.run(
            ['python', 'reciepe_n_health_assisstant/flow_recipe_recommendation.py', category, dietary_restrictions],
            text=True,          # Capture output as string
            capture_output=True # Capture both stdout and stderr
        )
