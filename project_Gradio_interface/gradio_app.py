...
def ingredient_breakdown(food):
    result = subprocess.run(
            ['python', 'project_Gradio_interface/flow_ingredient_breakdown.py', food],
...
def health_assistant(age, weight, activity_level):
    result = subprocess.run(
            ['python', 'project_Gradio_interface/flow_health_assistant.py', age, weight, activity_level],
            text=True,          # Capture output as string
            capture_output=True # Capture both stdout and stderr
        )
...
def recipe_recommendation(category, dietary_restrictions):
    result = subprocess.run(
            ['python', 'project_Gradio_interface/flow_recipe_recommendation.py', category, dietary_restrictions],
            text=True,          # Capture output as string
            capture_output=True # Capture both stdout and stderr
        )
    recipe = result.stdout
    return json.dumps({"recommended_recipe": str(recipe)})
...
def gradio_recipe_health_assistant():
    with gr.Blocks(css=custom_css) as app:
        ...
        recipe_button.click(
            lambda x, y: format_output(parse_json_output(recipe_recommendation(str(x), str(y)))),
